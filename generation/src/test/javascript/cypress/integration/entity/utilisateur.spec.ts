...
describe('Utilisateur e2e test', () => {
  ...
  const utilisateurSample = { pseudo: 'a vortals Towels', email: 'y/@/law', numeroTelephone: 'c indigo whiteboard' };
  ...
  describe('new Utilisateur page', () => {
    ...
    it('should create an instance of Utilisateur', () => {
      cy.get(`[data-cy="pseudo"]`).type('a cultivate').should('have.value', 'a cultivate');

      cy.get(`[data-cy="email"]`).type('htq/@/yg').should('have.value', 'htq/@/yg');

      cy.get(`[data-cy="numeroTelephone"]`).type('GB Chair').should('have.value', 'GB Chair');
