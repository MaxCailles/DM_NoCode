<template>
  <div>
    <h2 id="page-heading" data-cy="PayementHeading">
      <span v-text="$t('coopcycleApp.payement.home.title')" id="payement-heading">Payements</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('coopcycleApp.payement.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PayementCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('coopcycleApp.payement.home.createLabel')"> Create a new Payement </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && payements && payements.length === 0">
      <span v-text="$t('coopcycleApp.payement.home.notFound')">No payements found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="payements">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('coopcycleApp.payement.panier')">Panier</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="payement in payements" :key="payement.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="payement.panier">
                <router-link :to="{ name: 'PanierView', params: { panierId: payement.panier.id } }">{{ payement.panier.id }}</router-link>
              </div>
            </td>
            <td class="text-right">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="coopcycleApp.payement.delete.question" data-cy="payementDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-payement-heading" v-text="$t('coopcycleApp.payement.delete.question', { id: removeId })">
